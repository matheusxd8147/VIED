...
float min_corrente_a = 0;
float min_corrente_b = 0;
float min_corrente_c = 0;
float min_tensao_a = 0;
float min_tensao_b = 0;
float min_tensao_c = 0;
...
float teste[80],teste1[80],teste2[80];
static float pick_up;
double micro = 0 , sec = 0, mili = 0;
static float M, K, a, t, B = 1;
float tempo_inicial = 0;
struct timeval start_time;
struct timeval start1_time;
struct timeval start2_time;
struct timeval start3_time;
struct timeval stop_time;
struct timeval stop1_time;
struct timeval stop2_time;
struct timeval stop3_time;
struct timeval stop4_time;
struct timeval stop5_time;
struct timeval stop6_time;
float w = 2*pi*60;
float time_diff[12];
float an[6], teta[6];
...
svUpdateListener (SVSubscriber subscriber, void* parameter, SVSubscriber_ASDU asdu)
{
    ...
    if ((strcmp(svID,"IEDNameMU0101"))== 0){      
        ...
        if(((SVSubscriber_ASDU_getINT32 (asdu, 40)*0.01)>=0)){
                    if((SVSubscriber_ASDU_getINT32 (asdu, 40)*0.01)<min_tensao_b){
                    min_tensao_b = (SVSubscriber_ASDU_getINT32 (asdu, 40)*0.01);
    ...
    {   
        ...
        time_diff[0] = ((float)(stop1_time.tv_sec - start1_time.tv_sec));
        time_diff[0] += ((stop1_time.tv_usec - start1_time.tv_usec)/(float)MICRO_PER_SECOND);      
        time_diff[1] = ((float)(stop2_time.tv_sec - start1_time.tv_sec));
        time_diff[1] += ((stop2_time.tv_usec - start1_time.tv_usec)/(float)MICRO_PER_SECOND);       
        time_diff[2] = ((float)(stop3_time.tv_sec - start1_time.tv_sec));
        time_diff[2] += ((stop3_time.tv_usec - start1_time.tv_usec)/(float)MICRO_PER_SECOND);
        time_diff[3] = ((float)(stop4_time.tv_sec - start1_time.tv_sec));
        time_diff[3] += ((stop4_time.tv_usec - start1_time.tv_usec)/(float)MICRO_PER_SECOND);
        time_diff[4] = ((float)(stop5_time.tv_sec - start1_time.tv_sec));
        time_diff[4] += ((stop5_time.tv_usec - start1_time.tv_usec)/(float)MICRO_PER_SECOND);
        time_diff[5] = ((float)(stop6_time.tv_sec - start1_time.tv_sec));
        time_diff[5] += ((stop6_time.tv_usec - start1_time.tv_usec)/(float)MICRO_PER_SECOND);

        time_diff[6] = time_diff[0] - time_diff[1];
        time_diff[7] = time_diff[0] - time_diff[2];
        time_diff[8] = time_diff[0] - time_diff[3];
        time_diff[9] = time_diff[0] - time_diff[4];
        time_diff[10] = time_diff[0] - time_diff[5];

        teta[0] = 0;
        teta[1] = w * time_diff[6];
        teta[2] = w * time_diff[7];
        teta[3] = w * time_diff[8];
        teta[4] = w * time_diff[9];
        teta[5] = w * time_diff[10];
        an[0] = teta[0] * 180/pi;
        an[1] = teta[1] * 180/pi;
        an[2] = teta[2] * 180/pi;
        an[3] = teta[3] * 180/pi;
        ...
        if(an[1]<0){
            ...
            printf("   A tensão RMS da fase B no primário é: %.2f [V] %.1f°\n", tensao_primarioB, an[1] );
        ...
        if (t>0){
            ...
            { 
                ...
                time_diff[12] = (float)(stop_time.tv_sec - start_time.tv_sec);
                time_diff[12] += (stop_time.tv_usec - start_time.tv_usec)/(float)MICRO_PER_SECOND;
                //printf("\n%f s\n",time_diff[12]);
                //exit(0);
                if ((time_diff[12])>=t){
                    //printf("Corrente de curto-circuito.\nProteção Instantânea (50).\nAssinante Não Atuar.\nFalta Eliminada\n");
                    printf("-------------------------------------------------------------------------------------------------------------\n");            
                    printf("                                 ATUAR FUNÇÃO 51: SOBRECORRENTE TEMPORIZADA                                  \n");
                    printf("-------------------------------------------------------------------------------------------------------------\n");
