...
int main(int argc, char** argv) {
    ...
    if (error == IED_ERROR_OK) {
        ...
        MmsValue* value = IedConnection_readObject(con, &error, "SEL_751_1ANN/MVGGIO12.AnIn01.mag.f", IEC61850_FC_MX);
        ...
        IedConnection_writeObject(con, &error, "SEL_751_1CON/RBGGIO1.NamPlt.vendor", IEC61850_FC_DC, value);

        if (error != IED_ERROR_OK)
            printf("failed to write SEL_751_1CON/RBGGIO1.NamPlt.vendor! (error code: %i)\n", error);

        MmsValue_delete(value);

        /* read data set */
        ClientDataSet clientDataSet = IedConnection_readDataSetValues(con, &error, "SEL_751_1CFG/LLN0.DSet01", NULL);
        ...
        ClientReportControlBlock rcb =
                IedConnection_getRCBValues(con, &error, "SEL_751_1CFG/LLN0.BR.BRep0101", NULL);
        ...
        if (rcb) {
            ...
            IedConnection_installReportHandler(con, "SEL_751_1CFG/LLN0.BR.BRep0101",
