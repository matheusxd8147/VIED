...
svUpdateListener (SVSubscriber subscriber, void* parameter, SVSubscriber_ASDU asdu)
{
    ...
    float corrente_secundarioA = 0;
    float corrente_secundarioB = 0;
    float corrente_secundarioC = 0;
    int teste = 0;
...
/*     SVrms_delta = (SVrms_delta + pow(SVSubscriber_ASDU_getFLOAT32(asdu, 0),2));
    SVrms_estrela = (SVrms_estrela + pow(SVSubscriber_ASDU_getFLOAT32(asdu, 4),2));
    printf ("SVrms_delta: %f\n", SVrms_delta);
    printf ("SVrms_estrela: %f\n", SVrms_estrela);
    contadorSV += 1; */
    if (contadorSV1==80)
    {
        system ("clear");
       ...
       if ((abs(corrente_primarioA-corrente_secundarioA)/corrente_primarioA)*100>30)
        {
            ...
            printf("-------------------------------------------------------------------------------------------------------------\n");
            IedServer_updateDbposValue(iedServer, IEDMODEL_LD0_CSWI1_Pos_stVal, DBPOS_ON);
        ...
        if (teste == 0){
            IedServer_updateDbposValue(iedServer, IEDMODEL_LD0_CSWI1_Pos_stVal, DBPOS_OFF);
...
main(int argc, char** argv)
{
    ...
    GooseSubscriber subscriber = GooseSubscriber_create("SEL_7IEDNameMU020151_1CFG/LLN0$GO$GOOSE_SL_1", NULL); //Especificação de quem o ied irá receber as mensagens goose
